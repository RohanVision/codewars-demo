...
var debounce = function (fn, t) {
    ...
    return function (...args) {
        if (timeOutRef) {
            clearTimeout(timeOutRef); // cancelling setTimeout which is setting in future
        }
        timeOutRef = setTimeout(() => {
            fn(...args);
            clearTimeout(timeOutRef)
        }, t); // delayed the function by t miliseconds
    }
};
...
// i.debounce takes two arguments: fn and t.
//     ii. fn is the function that you want to debounce.
//         iii. t is the amount of time you want to wait before executing fn after the last time it was called.
//             iv. The debounce function returns a new function that takes any number of arguments(...args).
//                 v.Within the returned function, a timer is set using setTimeout.The timer is initially set to t milliseconds.
//                     vi.Every time the returned function is called, the clearTimeout function is called to reset the timer to t milliseconds.
//                         vii.Once the timer has elapsed without the returned function being called again, the timer's callback function is executed. The callback function calls fn with the arguments that were passed to the returned function.
// vii.The debounce function returns the new function that was created in step 2.
